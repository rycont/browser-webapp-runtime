...
import { patchRolldownWasm } from '../../src/patch-rolldown-wasm.ts'

/** 사용자 앱(Todo)이 import 하는 것들. memfs 의 node_modules 에 심긴다. */
const APP_TREE = ['react', 'react-dom', 'scheduler', 'tailwindcss']

/** rolldown wasm 부트스트랩 패치. 빈 객체면 원본 그대로. */
const WASM_PATCH = {}

export default defineConfig({
  root: import.meta.dirname,
  plugins: [patchRolldownWasm(WASM_PATCH), inlinePackages(APP_TREE, import.meta.url)],
